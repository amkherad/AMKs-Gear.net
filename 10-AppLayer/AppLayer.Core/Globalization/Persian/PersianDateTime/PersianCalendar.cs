...
namespace AMKsGear.AppLayer.Core.Globalization.Persian.PersianDateTime
{
    ...
    public class PersianCalendar : PersianCalendar<DefaultPersianDateTimeLocalization>{ }
    /// <summary>
    /// PersianCalendar calendar. Persian calendar, also named Jalaali calendar, was first based on Solar year by Omar Khayyam, the great Iranian poet, astrologer and scientist.
    /// Jalaali calendar is approximately 365 days. Each of the first six months in the Jalaali calendar has 31 days, each of the next five months has 30 days, and the last month has 29 days in a common year and 30 days in a leap year. A leap year is a year that, when divided by 33, has a remainder of 1, 5, 9, 13, 17, 22, 26, or 30. For example, the year 1370 is a leap year because dividing it by 33 yields a remainder of 17. There are approximately 8 leap years in every 33 year cycle.
    /// </summary>
    /// <see cref="http://www.codeproject.com/Articles/13664/Farsi-Library-Working-with-Dates-Calendars-and-Dat"/>
    public class PersianCalendar<TLocalization> : Calendar
        where TLocalization : class, IPersianDateTimeLocalization
    {
		/// <summary>
		...
		public override DateTime AddMonths(DateTime time, int months) 
		{
			if (Math.Abs(months) > 120000)
			{
			    throw new InvalidPersianDateException(
			        Localization.Format<IPersianDateTimeLocalization, TLocalization>(x => x.InvalidMonth));
			}
			...
			{
                throw new InvalidPersianDateException(
                    Localization.Format<IPersianDateTimeLocalization, TLocalization>(x => x.InvalidDateTime));
			}
		...
		public override DateTime ToDateTime(int year, int month, int day, int hour, int minute, int second, int millisecond, int era) 
		{
			...
			catch (ArgumentOutOfRangeException)
			{
                ...
                throw new InvalidPersianDateException(
                    Localization.Format<IPersianDateTimeLocalization, TLocalization>(x => x.InvalidMonthDay));
			}
		...
		public override int ToFourDigitYear(int year)
		{
			if (year != 0)
			{
				...
				catch (ArgumentOutOfRangeException)
				{
                    ...
                    throw new InvalidPersianDateException(
                    Localization.Format<IPersianDateTimeLocalization, TLocalization>(x => x.InvalidFourDigitYear));
				}
			}
		...
		private void CheckEraRange(bool validate, int era)
		{
			if (validate)
			{
                ...
                {
                    throw new InvalidPersianDateException(
                    Localization.Format<IPersianDateTimeLocalization, TLocalization>(x => x.InvalidEra));
                }
			}
			return;
		}
		...
		private void CheckYearRange(bool validate, int year) 
		{
			if (validate)
			{
                ...
                {
                    throw new InvalidPersianDateException(
                    Localization.Format<IPersianDateTimeLocalization, TLocalization>(x => x.InvalidYear));
                }
			}
			return;
		}
		...
		private void CheckDateRange(bool validate, int year, int month, int day)
		{
			if (validate)
			{
				...
				if (day < 1 || maxday < day)
				{
                    ...
                    {
                        throw new InvalidPersianDateException(
                    Localization.Format<IPersianDateTimeLocalization, TLocalization>(x => x.InvalidLeapYear));
                    }
                    else
                    {
                        throw new InvalidPersianDateException(
                    Localization.Format<IPersianDateTimeLocalization, TLocalization>(x => x.InvalidDay));
                    }
				}
			}
		}
